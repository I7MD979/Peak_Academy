...
// ── POST handler: Google OAuth form POST ──────────────────────────────────
export async function POST(request) {
  ...
  try {
    ...
    let redirectPath = (await resolvePostAuthPath(sessionData.access_token, request)) || "/onboarding";

    if (returnTo && redirectPath !== "/auth/login") {
      if (redirectPath === "/onboarding") {
        redirectPath = appendNextParam("/onboarding", returnTo);
      } else {
