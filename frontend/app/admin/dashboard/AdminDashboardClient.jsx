...
export default function AdminDashboardClient({ initialDashboard = null }) {
  const profile = useSidebarProfile();
  const { user: authUser } = useAuth();
  const currentUserId = authUser?.id || "";

  const [stats, setStats] = useState(initialDashboard?.stats || null);
  const [recentUsers, setRecentUsers] = useState(initialDashboard?.recent_users || []);
  const [recentSessions, setRecentSessions] = useState(initialDashboard?.recent_sessions || []);
