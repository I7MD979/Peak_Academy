/**
 * Applies 20260706_verification_security.sql to remote Supabase Postgres.
 */
...
const migrationPath = path.join(
  __dirname,
  "../supabase/migrations/20260706_verification_security.sql"
);
...
async function verify() {
  ...
  const { error } = await supabase.from("verification_audit_log").select("id").limit(1);
  if (error) throw new Error(`Verify failed: ${error.message}`);
  console.log("Verified: verification_audit_log table is available.");
}
