/**
 * Applies 20260706_identity_verification.sql to remote Supabase Postgres.
 */
...
const migrationPath = path.join(
  __dirname,
  "../supabase/migrations/20260706_identity_verification.sql"
);
...
async function verify() {
  ...
  const { error } = await supabase.from("verification_documents").select("id").limit(1);
  if (error) throw new Error(`Verify failed: ${error.message}`);
  const { data: buckets } = await supabase.storage.listBuckets();
  if (!buckets?.some((b) => b.id === "verification-docs")) {
    throw new Error("Verify failed: verification-docs bucket not found");
  }
  console.log("Verified: verification_documents table and verification-docs bucket are available.");
}
